  ...
  const unsubscribe = logger.subscribe((level, message, fields) => {
    ...
    logs.push({ id: ++sequence, at: Date.now(), level: level.toUpperCase(), message,
      ...
      detail: ['launch pad test failed','care package test failed','care package launch failed','movement debug path failed'].includes(message) && typeof fields.reason === 'string' ? fields.reason :
        ...
        : message === 'care package hologram'
          ? `status=${typeof fields.status === 'string' ? fields.status : '?'} remaining=${typeof fields.clicksRemaining === 'number' ? fields.clicksRemaining : '?'}`
        : message === 'care package interaction failed'
