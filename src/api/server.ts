  ...
  const unsubscribe = logger.subscribe((level, message, fields) => {
    ...
    logs.push({ id: ++sequence, at: Date.now(), level: level.toUpperCase(), message,
      ...
      detail: ['launch pad test failed','care package test failed','care package launch failed','movement debug path failed'].includes(message) && typeof fields.reason === 'string' ? fields.reason :
        ...
        : message === 'care package priority loot'
          ? `clicked=${typeof fields.clicked === 'number' ? fields.clicked : '?'} items=${typeof fields.items === 'string' ? fields.items : '?'}`
        : message === 'care package knocked out of reach; returning' || message === 'care package reach restored'
          ? `recovery=${typeof fields.recovery === 'number' ? fields.recovery : '?'}`
        : message === 'pit path planned'
          ? `cache=${typeof fields.cacheStatus === 'string' ? fields.cacheStatus : '?'} chunks=${typeof fields.scannedChunks === 'number' ? fields.scannedChunks : '?'} dynamic=${typeof fields.dynamicBlocks === 'number' ? fields.dynamicBlocks : '?'} overlayRev=${typeof fields.overlayRevision === 'number' ? fields.overlayRevision : '?'} nodes=${typeof fields.expandedNodes === 'number' ? fields.expandedNodes : '?'} time=${typeof fields.planningMs === 'number' ? fields.planningMs : '?'}ms`
        : message === 'pit path replan requested'
          ? `reason=${typeof fields.reason === 'string' ? fields.reason : '?'} avoided=${typeof fields.avoidedColumns === 'number' ? fields.avoidedColumns : '?'} waypoint=${typeof fields.waypointX === 'number' ? fields.waypointX.toFixed(1) : '?'},${typeof fields.waypointY === 'number' ? fields.waypointY.toFixed(1) : '?'},${typeof fields.waypointZ === 'number' ? fields.waypointZ.toFixed(1) : '?'}`
        : message === 'pit navigation prewarm completed'
          ? `cache=${typeof fields.cacheStatus === 'string' ? fields.cacheStatus : '?'} chunks=${typeof fields.scannedChunks === 'number' ? fields.scannedChunks : '?'} dynamic=${typeof fields.dynamicBlocks === 'number' ? fields.dynamicBlocks : '?'} fingerprint=${typeof fields.fingerprint === 'string' ? fields.fingerprint : '?'}`
        : message === 'pit navigation prewarm failed'
        ...
        : message === 'server position correction'
          ? `Δh=${typeof fields.horizontal === 'number' ? fields.horizontal.toFixed(3) : '?'} Δy=${typeof fields.vertical === 'number' ? fields.vertical.toFixed(3) : '?'} ` +
            ...
            `rel=${Boolean(fields.relativeX)?'X':'-'}${Boolean(fields.relativeY)?'Y':'-'}${Boolean(fields.relativeZ)?'Z':'-'} floor=${typeof fields.floorBlock === 'string' ? fields.floorBlock : '?'}:${typeof fields.floorMeta === 'number' ? fields.floorMeta : '?'} feet=${typeof fields.feetBlock === 'string' ? fields.feetBlock : '?'}:${typeof fields.feetMeta === 'number' ? fields.feetMeta : '?'} supportBefore=${typeof fields.beforeSupport === 'string' ? fields.beforeSupport : '?'} supportTarget=${typeof fields.targetSupport === 'string' ? fields.targetSupport : '?'} ` +
