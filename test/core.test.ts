...
test('configuration defaults are safe and malformed values fail closed', () => {
  ...
  assert.equal(loadConfig({}).pitEventMinPlayers, 21);
  assert.equal(loadConfig({}).pitPopulationCheckMs, 10000);
  ...
  for (const env of [{ BOT_COUNT: '21' }, { BOT_COUNT: '1.5' }, { PIT_EVENT_MIN_PLAYERS: '-1' }, { PIT_POPULATION_CHECK_MS: '999' }, { PATH_CONCURRENCY: '0' }, { DEBUG: 'yes' }, { MODE: 'production' }, { LOBBY_COMMAND: '/server pit' }, { TRANSFER_MESSAGE_CHANNEL: 'title' },
